# Appendix 1: Pre-Survey Questionnaire (Revised for LYDO Connect)

Republic of the Philippines  
POLYTECHNIC UNIVERSITY OF THE PHILIPPINES  
OFFICE OF THE VICE PRESIDENT FOR ACADEMIC AFFAIRS  
College of Computer and Information Sciences

## PRE-SURVEY QUESTIONNAIRE

**LYDO Connect: A Progressive Web Application for Information, Participation, and Transparency for Local Youth Development Offices**

Good day!

You are invited to participate in a research study about **LYDO Connect**. This pre-survey aims to understand your current experience in accessing youth-related information and services such as programs, events, organization information, transparency records, and citizen concern reporting.

Your participation is voluntary, and your responses will be treated with strict confidentiality. The email address collected will be used only for coordination related to system testing and the post-survey phase. You may withdraw at any time without penalty.

By proceeding, you confirm that you understand the purpose of the study and agree to participate.

## [Screening Question]

1. Are you a youth participant or citizen who may use online youth-related services (e.g., viewing programs/events, registration, organization info, transparency pages, or concern reporting)?
- Yes
- No (If No, the survey will automatically end)

## [Email Address]

Please provide a valid email address. This will be used only to coordinate system testing and the post-survey phase.

Email: ___________________________

## [Section A: Respondent Profile]

2. Age Group:
- 15-17
- 18-24
- 25-30
- 31 and above
- Prefer not to say

3. Sex:
- Male
- Female
- Prefer not to say

4. Role:
- Youth participant / student
- General citizen
- Others: _______________

5. Barangay / City of residence (optional):
_______________________

6. Have you previously used any online platform for youth programs, event registration, or citizen concern reporting?
- Yes
- No

## [Section B: Current Practices in Accessing Youth-Related Information and Services]

7. What sources do you currently use to find youth-related announcements, program/event details, organization information, or public records? (Select all that apply)
- Official LGU / LYDO websites
- Official Facebook pages
- Barangay / SK pages
- School announcements
- Friends / peers
- Community groups / chat groups
- Walk-in inquiry at offices
- Others: _______________

8. On average, how much time do you spend searching for youth-related information or services online?
- Less than 1 hour per week
- 1-3 hours per week
- 3-5 hours per week
- More than 5 hours per week

9. Which activities are currently most difficult for you? (Select up to 3)
- Finding complete program details
- Tracking event schedules and venues
- Registering for programs/events
- Verifying youth organization information
- Accessing transparency and financial disclosures
- Submitting or tracking concerns/tickets
- Others: _______________

## [Section C: Baseline Perception and Challenges]

Directions: Please indicate your level of agreement with the following statements.

**Likert Scale:** 5 = Strongly Agree, 4 = Agree, 3 = Neutral, 2 = Disagree, 1 = Strongly Disagree

1. I find it difficult to locate complete and updated youth program information online.
2. It is easy for me to find accurate event schedules, venues, and registration instructions.
3. Information from different youth-related sources is often inconsistent.
4. Registering for programs or events is time-consuming.
5. I need one platform where I can view programs, events, and organization information in one place.
6. I can easily verify whether youth organization information is credible and source-backed.
7. Accessing transparency and financial disclosure records is straightforward.
8. I know how to submit and track a youth-related concern or service request online.
9. I am confident I can monitor my own participation history in youth activities.
10. I believe account security and privacy are important in youth service portals.

## [Section D: Open-Ended Questions]

10. What is your biggest challenge when accessing youth-related programs, events, transparency records, or citizen services?

________________________________________________________________________

11. What features would you expect from an effective youth engagement platform like LYDO Connect?

________________________________________________________________________

## [End-of-Survey Notice]

Thank you for completing the pre-survey.

The LYDO Connect system testing link and post-survey questionnaire will be sent to the email address you provided and/or posted on the same platform where this survey was shared. Please monitor these channels for further instructions for the next phase of the study.


