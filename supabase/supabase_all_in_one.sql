...
create table if not exists public.organization_profiles (
  ...
  directory_show_representative boolean not null default false,
  directory_show_adviser boolean not null default false,
  directory_visibility_initialized boolean not null default false,
...
-- Privacy-safe, opt-in organization directory for authenticated PWA users.
update public.organization_profiles
set directory_visibility = true,
...
as $$
begin
  if auth.uid() is null then raise exception 'Please sign in with your organization account first.'; end if;
  update public.organization_profiles
  set directory_visibility = coalesce(_visible, false),
      directory_show_representative = coalesce(_show_representative, false),
      directory_show_adviser = coalesce(_show_adviser, false),
      updated_at = now()
  where user_id = auth.uid();
  if not found then raise exception 'Organization profile not found.'; end if;
  return query select * from public.organization_profiles where user_id = auth.uid();
end;
$$;

create or replace function public.search_public_organizations()
returns table (
  organization_id uuid, organization_name text, profile_image_url text,
  major_classification text, sub_classification text, district text,
  barangay text, advocacies text[], facebook_page_url text,
  verified_at timestamptz, yorp_registered_year smallint,
  representative_name text, adviser_name text
)
...
begin
  ...
  return query
  select op.id, op.organization_name, coalesce(op.profile_image_url, ''),
    coalesce(op.major_classification, ''), coalesce(op.sub_classification, ''),
    coalesce(op.district, ''), coalesce(op.barangay, ''),
    coalesce(op.advocacies, array[]::text[]), coalesce(op.facebook_page_url, ''),
    op.verified_at, op.yorp_registered_year,
...
returns table (
  ...
  barangay text, advocacies text[], facebook_page_url text,
  verified_at timestamptz, yorp_registered_year smallint,
  representative_name text, adviser_name text, activities jsonb
)
language plpgsql stable security definer set search_path = public
as $$
begin
    ...
    (select coalesce(jsonb_agg(activity order by activity ->> 'date' desc), '[]'::jsonb)
     from (
       select jsonb_build_object(
         'id', yep.id, 'name', coalesce(yca.name, yep.activity_name),
         'date', coalesce(yca.date, yep.activity_date),
         'venue', coalesce(yca.venue, yep.venue), 'kind', 'city_led'
       ) activity
       from public.ypop_event_participations yep
       left join public.ypop_city_activities yca on yca.id = yep.activity_id
       where yep.organization_id = op.id and yep.status = 'verified'
       union all
       select jsonb_build_object(
         'id', yoa.id, 'name', yoa.activity_name, 'date', yoa.activity_date,
         'venue', yoa.venue, 'kind', 'organization_led'
       )
       from public.ypop_org_activities yoa
       where yoa.organization_id = op.id and yoa.status = 'approved'
     ) safe_activities)
  from public.organization_profiles op
